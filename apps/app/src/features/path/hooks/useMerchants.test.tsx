...
describe("useMerchants", () => {
  ...
  it("defaults the results limit to 25 in the invoke body", async () => {
    invokeMock.mockResolvedValue({ data: { prospects: [] }, error: null });
    renderHook(() => useMerchants({ lat: 30.2672, lng: -97.7431 }), { wrapper });
    await waitFor(() => expect(invokeMock).toHaveBeenCalled());
    expect(invokeMock.mock.calls[0]![1].body).toMatchObject({ limit: 25 });
  });

  it("passes a custom results limit through", async () => {
    invokeMock.mockResolvedValue({ data: { prospects: [] }, error: null });
    renderHook(() => useMerchants({ lat: 30.2672, lng: -97.7431 }, { limit: 40 }), { wrapper });
    await waitFor(() => expect(invokeMock).toHaveBeenCalled());
    expect(invokeMock.mock.calls[0]![1].body).toMatchObject({ limit: 40 });
  });

  it("clamps the results limit to 50 when higher", async () => {
    invokeMock.mockResolvedValue({ data: { prospects: [] }, error: null });
    renderHook(() => useMerchants({ lat: 30.2672, lng: -97.7431 }, { limit: 100 }), { wrapper });
    await waitFor(() => expect(invokeMock).toHaveBeenCalled());
    expect(invokeMock.mock.calls[0]![1].body).toMatchObject({ limit: 50 });
  });

  it("clamps a below-range results limit to 1", async () => {
    invokeMock.mockResolvedValue({ data: { prospects: [] }, error: null });
    renderHook(() => useMerchants({ lat: 30.2672, lng: -97.7431 }, { limit: 0 }), { wrapper });
    await waitFor(() => expect(invokeMock).toHaveBeenCalled());
    expect(invokeMock.mock.calls[0]![1].body).toMatchObject({ limit: 25 });
  });
});
