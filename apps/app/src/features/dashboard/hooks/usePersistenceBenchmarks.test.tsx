...
describe("usePersistenceBenchmarks", () => {
  it("derives benchmarks, peer sub-component averages, and a scope label", () => {
    mockRows = [
      { ownerId: "a", composite: 60, followUpPoints: 20, cadencePoints: 15, reEngagementPoints: 12 },
      { ownerId: "b", composite: 80, followUpPoints: 40, cadencePoints: 30, reEngagementPoints: 18 },
    ];
    mockRole = "manager";
    ...
    // median(12, 18) = 15 -> 15/30 = 50%
    expect(result.current.reEngagementAvgPct).toBe(50);
    expect(result.current.avgLabel).toBe("Team average");
  });
  it("solo for a single rep (rep scope)", () => {
    mockRows = [{ ownerId: "a", composite: 70, followUpPoints: 30, cadencePoints: 20, reEngagementPoints: 15 }];
    const { result } = renderHook(() => usePersistenceBenchmarks());
