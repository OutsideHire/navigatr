-- Demo reset: make the curated fixture multi-org safe.
--
-- The fixture previously inserted hardcoded deal/partner ids
-- (a0000000-...-NN / b0000000-...-NN). Those ids are globally unique
-- (deals_pkey / partners_pkey are on id alone), so once one org ran the reset
-- and created them, a SECOND org running the reset hit
-- "duplicate key value violates unique constraint deals_pkey" -- its wipe only
-- clears its own org_id rows, but the insert still tried to reuse the same ids
-- already owned by the first org.
--
-- Fix: derive every fixture id per-org as md5(v_org_id::text || '<template id>')::uuid.
-- Deterministic (a given org always gets the same ids, so repeat resets are
-- idempotent) and disjoint across orgs (different org_id -> different ids), so
-- any number of opted-in orgs can each hold their own demo set independently.
-- Also carries the current partner-type keys (cpa_bookkeeper /
-- business_banker_commercial_lender / small_business_attorney).
--
-- Apply via the Supabase SQL editor. Function-only change: no schema/table
-- change, no frontend dependency, no push-ordering constraint.
