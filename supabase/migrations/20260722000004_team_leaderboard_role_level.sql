-- team_leaderboard: expose role_level (PRD 6.8.A, phase 1B.3). Re-runnable.
--
-- The Team page renders an org-chart tree and the agent-detail page shows a
-- role-level selector; both need each member's role_level. team_leaderboard is
-- the roster query that drives those screens, so add role_level to its return
-- (right after role) and to both select branches. Return-type change requires
-- DROP. Body is otherwise verbatim from 20260714000004; only role_level is
-- added (3 matched positions) - no aggregate / filter / gate change.
