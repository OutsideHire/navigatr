-- 20260524000001_team_leaderboard.sql
--
-- Read-only aggregate RPC for the admin team-leaderboard UI.  Returns one row
-- per profile (active or deactivated) in the caller's org, plus one row per
-- pending invite — matching the merged-list shape already used by useOrgAgents
-- so the frontend can drive both panels from a single query result.
--
-- Authz: caller must be an authenticated manager or admin (not deactivated).
-- No writes; SECURITY DEFINER is used purely to lock down row visibility to
-- the caller's org without exposing RLS policies to the client.
-- See docs/superpowers/plans/2026-05-22-iso-admin-portal.md, Task A.

-- ---------------------------------------------------------------------------
-- team_leaderboard(p_window_days int default 30)
--
-- Returns per-agent rollup columns:
--   open_deals / pipeline_cents    — deals in non-won stages, all time
...
--
-- p_window_days is clamped to [1, 365] so a bad caller can't accidentally pull
-- all-time won data when they mean "last 30 days".  Values ≤ 0 or null are
-- normalised to 30.
--
-- Performance: all aggregation happens in two CTEs (deal_aggs, activity_aggs)
-- that each scan once with a WHERE on (org_id) — covered by the existing
-- composite indexes on deals(org_id, owner_id) and
-- activities(org_id, logged_by, occurred_at).  No PL/pgSQL loop.
-- ---------------------------------------------------------------------------
...
begin
  ...
  deal_aggs as (
    select
      d.owner_id,
      count(*)        filter (where d.stage != 'won')                                                           as open_count,
      coalesce(
