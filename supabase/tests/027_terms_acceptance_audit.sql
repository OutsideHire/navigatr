...
--      current document versions and the user's id/org (no PII stored);
...
--   3. no app role can read or write the audit table (ops/legal-only via the
--      service role; rows come only from the SECURITY DEFINER trigger).
...
declare
  n int;
  v_terms text; v_privacy text; v_org uuid;
begin
  ...
  select terms_version, privacy_version, org_id
    into v_terms, v_privacy, v_org
...
-- ── 3) No app role can read or write the audit table directly. ──
do $$
begin
  if has_table_privilege('authenticated', 'public.terms_acceptances', 'SELECT') then
    raise exception 'authenticated must NOT have SELECT on terms_acceptances (ops/legal-only)';
  end if;
